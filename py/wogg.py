...
def get_k(text,type):
    key = ''
    # cates = ["class", "area", "lang", "year", "letter", "by", "sort"]
    ...
    for i,x in cates.items():
        ...
        if type == 'wobg':
            if not key:
                if 'id' in text:
                    key = 'id'
    return key.replace('全部','')

def get_v(text,key,type,tid):
    if type == 'wobg':
        return text.split(f'{key}/')[-1].split('/')[0].split('.')[0]
    else:
        v=text.split('/',-1)[-1].split('.')[0].replace('-','')
        if v.startswith(tid):
            v = v[len(tid):]
        if v=='09':v='0-9'
        return v
...
async def get_ft(session, url,type,tid):
    print(f"请求: {url}")
    ...
    for i in list(data('div.library-box.scroll-box').items())[1:]:
        ...
        ft.append({
            'name': key,
            'key': key,
            'value': [{'v': unquote(get_v(j.attr('href'),key,type,tid)), 'n': j.text()} for j in c.items()]
        })
    return ft
    ...
    async with aiohttp.ClientSession() as session:
        ...
        if len(categories):
            for i in categories:
                ...
                task = asyncio.create_task(get_ft(session, f"{host}{path}",type,i['type_id']))
...
if __name__ == '__main__':
    ...
    # https://erxiaofn.click/index.php/vod/show/area/%E5%86%85%E5%9C%B0/by/hits/class/%E6%88%98%E4%BA%89/id/13/lang/%E5%9B%BD%E8%AF%AD/letter/B/year/2024.html